...
RUN apt -qq update && DEBIAN_FRONTEND=noninteractive apt -qq install -y aria2 curl ffmpeg fuse3 htop inotify-tools jq less libchromaprint-tools mediainfo mkvtoolnix nano ncdu novnc openssh-client openssh-server parallel postgresql-client python3-pip python3-websockify qbittorrent-nox rename sudo sqlite3 tigervnc-standalone-server tigervnc-xorg-extension tmate tmux tzdata unzip xfce4-terminal xserver-xorg-video-dummy xubuntu-core^ && apt -qq full-upgrade -y && for a in autoremove purge clean; do apt -yqq $a; done; rm -rf /var/lib/apt/lists/*
