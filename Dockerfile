...
RUN (apt -qq update && DEBIAN_FRONTEND=noninteractive apt -yqq install aria2 curl ffmpeg fuse3 htop inotify-tools jq less libchromaprint-tools mediainfo mkvtoolnix nano ncdu novnc openssh-client openssh-server parallel postgresql-client python3-pip python3-websockify qbittorrent-nox rename sudo sqlite3 tigervnc-standalone-server tigervnc-xorg-extension tmux tzdata unzip xfce4-terminal xserver-xorg-video-dummy dbus-x11 && sudo apt -yqq install placeholder_for_desktop_package && apt -yqq full-upgrade && for a in autoremove purge clean; do apt -yqq $a; done) > /dev/null 2>&1 && rm -rf /var/lib/apt/lists/* /var/cache/apt/archives/*
